...
class _MyPainter extends State<MyPainter> {
  ...
  void initState() {
    super.initState();
     ...
     controller =
        AnimationController(duration: Duration(seconds: 10),vsync: this.controller);

    var animation = Tween<double>(begin: 0, end: 300).animate(controller)
      ..addListener(() {
        setState(() {});
      })
    ...
    this.particles = List.generate(100, (index) {
      var p = MyParticle(position: Offset(-1,-1), color: getRandomColor(rgn), speed: rgn.nextDouble()*maxSpeed, theta: rgn.nextDouble()*maxTheta, radius: rgn.nextDouble()*maxRadius);
